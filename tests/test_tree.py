...
def test_tree_node_clone():
    ...
    # Can also clone the entire
    for i in range(-5, 0):
        tree.insert(i)
    assert tree.clone().find(-3) is not None
...
def test_tree_node_visit_preorder():
    ...
    # Can stop at all nodes
    for stopIdx in values:

        def stop_visit(node: BinarySearchTree, depth, data):
            if node.key == stopIdx:
                return STOP
            return None

        assert tree.visit_preorder(stop_visit) == STOP
...
def test_tree_node_visit_inorder():
    ...
    for stopIdx in values:
        ...
        assert tree.visit_inorder(stop_visit) == STOP
...
def test_tree_node_visit_postorder():
    ...
    for stopIdx in values:
        ...
        assert tree.visit_postorder(stop_visit) == STOP
